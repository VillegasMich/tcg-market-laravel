        ...
        <div class="group relative w-fit">
          <div
            ...
            <div class="grid gap-x-4 gap-y-4 grid-cols-3 grid-rows-3 overflow-hidden">
              ...
              @if ($item->TCGCard)
              <img class="bg-black w-14 h-20 rounded-sm" src="{{ asset('/storage/' . $item->TCGCard->image) }}" alt="">
              @endif
              @endforeach
            </div>
          </div>
          <div class="mt-4 flex justify-between">
            <div class="w-full">
              <h3 class="text-sm w-full text-center text-gray-700 flex flex-col justify-center">
                <small class="text-sm text-gray-500">Status: {{$order->getStatus()}}</small>
                <small class="text-sm text-gray-500">{{$order->items->count()}} items</small>
                <small class="text-sm text-gray-500">Total: ${{$order->getTotal()}}</small>
